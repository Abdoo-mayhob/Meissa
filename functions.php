...
function dd($var, $display = false, $msg = '')
{
    ...
    echo "<pre $display_class class='dd'>$msg ";
    var_dump($var);
    echo '</pre>';
...
add_action('init', function () {
    if (!($_GET['clear_meissa_cache'] ?? false))
        return;
...
add_action('save_post', function ($post_id, $post, $updating) {
    ...
    if ($post->post_status == 'auto-draft')
        return; // an auto-draft is generated on the hit of "Add New Post"
    ...
    if ($ots == 'terminal_slash') {
        ...
        foreach ($posts_array as $post) {
            ...
            foreach ($tags as $tag) {
                ...
                if (str_contains($href, 'meissa.space') == false)
                    continue;
                if (substr($href, -1) == '/')
                    continue;
...
add_shortcode('ali-alkhatib', function () {
    ?>
    <style>
        hl {
            color: var(--clr-pri);
    ...
    <h2 style="font-size: var(--fs-ter);">Chief Editor &amp; Content Manager | مدير المحتوى و التحرير</h2>
    <br>
    <a href="https://www.facebook.com/rain.maker89">Ali's Facebook</a>
    <br>

    <h3>عن علي</h3>
    <ul id="ali">
        <li>
            له الدور الأساسي في ترجمة, تحرير و تدقيق
            <hl>
                اكثر من 363 مقال علمي
            </hl>
            في مدونة ميسان
        </li>
        <li>
            تهيئة وضبط المحتوى لأعلى معايير الـ SEO
        </li>
        <li>
            الادارة و الإشراف على عمليات السوشل ميديا و التسويق عبرها
        </li>
        <li>اكثر من
            <hl> 363
            </hl>
    ...
    <p>
        تتقدم ادارة وفريق ميسان بأعلى درجات الشكر و التقدير لجهود علي واسهاماته العميقة و القيمة في المدونة ومحتواها و
        القيمة المقدمة للقراء في فترة عمل المدونة بين يوليو 2022 و يناير 2025
    </p>
    <?php
});